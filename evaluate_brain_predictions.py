...
def evaluate(input_path, output_path, subject):
    start_time = tm.time()
    ...
    neighborhoods = np.load('./data/voxel_neighborhoods/' + subject + '_ars_auto2.npy')
    ...
    for ind_num in range(n_folds):
        ...
        accs[ind_num, :], _, _, _ = binary_classify_neighborhoods(preds_t_per_feat[test_ind, :],
                                                                  test_t_per_feat[test_ind, :], n_class=20,
                                                                  nSample=1000, pair_samples=[],
